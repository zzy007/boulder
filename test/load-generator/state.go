...
func parse (der []byte)  (key *ecdsa.PrivateKey, err error)  {
	var privKey ecPrivateKey
	if _, err := asn1.Unmarshal(der, &privKey); err != nil {
		if _, err := asn1.Unmarshal(der, &pkcs8{}); err == nil {
			return nil, fmt.Errorf("[zzy-debug]load-generator: failed to parse private key (use ParsePKCS8PrivateKey instead for this key format)")
		}
		if _, err := asn1.Unmarshal(der, &pkcs1PrivateKey{}); err == nil {
			return nil,  fmt.Errorf("[zzy-debug]load-generator: failed to parse private key (use ParsePKCS1PrivateKey instead for this key format)")
		}
		return nil,  fmt.Errorf("[zzy-debug]load-generator: failed to parse EC private key: " + err.Error())
	}
	if privKey.Version != 1 {
		return nil, fmt.Errorf("[zzy-debug]load-generator: unknown EC private key version %d", privKey.Version)
	}
	...
	if k.Cmp(curveOrder) >= 0 {
		return nil, fmt.Errorf("[zzy-debug]load-generator: invalid elliptic curve private key value")
	}
	...
	for len(privKey.PrivateKey) > len(privateKey) {
		if privKey.PrivateKey[0] != 0 {
			return nil, fmt.Errorf("[zzy-debug]load-generator err: invalid private key length")
		}
...
func marshalECPrivateKeyWithOID(key *ecdsa.PrivateKey) ([]byte, error) {
...
func (s *State) Snapshot(filename string) error {
	...
	for _, acct := range s.accts {
		k, err := marshalECPrivateKeyWithOID(acct.key)
		if err != nil {
			return err
		}
