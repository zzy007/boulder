...
func (is *integrationSrv) addChainOrPre(w http.ResponseWriter, r *http.Request, precert bool) {
	...
	cert, err := sm2.ParseCertificate(b)
	if err != nil {
		w.WriteHeader(4001)
		return
	}
...
func parse (der []byte)  (key *ecdsa.PrivateKey, err error)  {
	...
	if _, err := asn1.Unmarshal(der, &privKey); err != nil {
		if _, err := asn1.Unmarshal(der, &pkcs8{}); err == nil {
			return nil, fmt.Errorf("x509: failed to parse private key (use ParsePKCS8PrivateKey instead for this key format)")
		}
		if _, err := asn1.Unmarshal(der, &pkcs1PrivateKey{}); err == nil {
			return nil,  fmt.Errorf("x509: failed to parse private key (use ParsePKCS1PrivateKey instead for this key format)")
		}
...
func runPersonality(p Personality) {
	...
	pubKeyBytes, err := sm2.MarshalPKIXPublicKey(&key.PublicKey)
	if err != nil {
		log.Fatal(err)
	}
	is := integrationSrv{
