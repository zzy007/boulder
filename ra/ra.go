...
func (ra *RegistrationAuthorityImpl) MatchesCSR(parsedCertificate *sm2.Certificate, csr *sm2.CertificateRequest) error {
	...
	if !reflect.DeepEqual(parsedCertificate.ExtKeyUsage, []sm2.ExtKeyUsage{sm2.ExtKeyUsageServerAuth, sm2.ExtKeyUsageClientAuth}) {
...
func (ra *RegistrationAuthorityImpl) FinalizeOrder(ctx context.Context, req *rapb.FinalizeOrderRequest) (*corepb.Order, error) {
	...
	csrOb, err := sm2.ParseCertificateRequest(req.Csr)
	if err != nil {
		return nil, err
	}
	...
	parsedCertificate, err := sm2.ParseCertificate([]byte(cert.DER))
	if err != nil {
...
func (ra *RegistrationAuthorityImpl) issueCertificateInner(
	...
	parsedPrecert, err := sm2.ParseCertificate(precert.DER)
	if err != nil {
...
func (ra *RegistrationAuthorityImpl) getSCTs(ctx context.Context, cert []byte, expiration time.Time) (core.SCTDERs, error) {
	...
	if err != nil {
		...
		if err == context.DeadlineExceeded {
			state = "[zzy-debug ra 1251]deadlineExceeded"
...
func (ra *RegistrationAuthorityImpl) revokeCertificate(ctx context.Context, cert sm2.Certificate, code revocation.Reason, revokedBy int64, source string, comment string) error {
	reason := int32(code)
...
func (ra *RegistrationAuthorityImpl) RevokeCertificateWithReg(ctx context.Context, cert sm2.Certificate, revocationCode revocation.Reason, regID int64) error {
...
func (ra *RegistrationAuthorityImpl) AdministrativelyRevokeCertificate(ctx context.Context, cert sm2.Certificate, revocationCode revocation.Reason, user string) error {
	serialString := core.SerialToString(cert.SerialNumber)
