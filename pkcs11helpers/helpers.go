...
func (s *Session) getPublicKeyID(label string, publicKey crypto.PublicKey) ([]byte, error) {
	...
	case *ecdsa.PublicKey :
		...
		rawValue := asn1.RawValue{
			...
			Bytes: elliptic.Marshal(sm2.P256Sm2(), key.X, key.Y),
		}
		marshalledPoint, err := asn1.Marshal(rawValue)
		if err != nil {
			return nil, err
		}
		curveOID, err := asn1.Marshal(curveOIDs["SM2-P-256"])
		if err != nil {
			return nil, err
		}
		template = []*pkcs11.Attribute{
			pkcs11.NewAttribute(pkcs11.CKA_CLASS, pkcs11.CKO_PUBLIC_KEY),
			pkcs11.NewAttribute(pkcs11.CKA_LABEL, []byte(label)),
			pkcs11.NewAttribute(pkcs11.CKA_KEY_TYPE, pkcs11.CKK_IBM_SM2),
	...
	default:
		return nil, fmt.Errorf("[zzy-debug helpers.go 126] unsupported public key of type %T", publicKey)
	}
...
func (p *x509Signer) Sign(rand io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[zzy-debug helpers 172] X509Signer Sign err %q,hash is %q",err,opts.HashFunc())
	}
...
func (s *Session) GenerateKeyPair(m []*pkcs11.Mechanism, pubAttrs []*pkcs11.Attribute, privAttrs []*pkcs11.Attribute) (pkcs11.ObjectHandle, pkcs11.ObjectHandle, error) {
	...
	//return s.Module.GenerateKeyPairForSM2(s.Session, m, pubAttrs, privAttrs)
}
...
//	"06052B81040021":       sm2.P256Sm2(),
//	"001003084000021":       elliptic.P224(),
//	"0102348": elliptic.P256(),
...
func (s *Session) GetECDSAPublicKey(object pkcs11.ObjectHandle) (*sm2.PublicKey, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[zzy-debug pkcs11helpers helpers 232]Failed to retrieve key attributes: %s", err)
	}
	...
	if pointBytes == nil || pubKey.Curve == nil {
		return nil, errors.New("[zzy-debug pkcs11helpers helpers 252]Couldn't retrieve EC point and EC parameters")
	}
	...
	if x == nil {
		...
		if err != nil {
			return nil, fmt.Errorf("[zzy-debug pkcs11helpers helpers 271]Failed to unmarshal returned CKA_EC_POINT: %s", err)
		}
		if len(point.Bytes) == 0 {
			return nil, errors.New("[zzy-debug pkcs11helpers helpers 274]Invalid CKA_EC_POINT value returned, OCTET string is empty")
		}
		...
		if x == nil {
			return nil, errors.New("[zzy-debug pkcs11helpers helpers 283]Invalid CKA_EC_POINT value returned, point is malformed")
		}
	}
...
func (s *Session) Sign(object pkcs11.ObjectHandle, keyType keyType, digest []byte, hash crypto.Hash) ([]byte, error) {
	...
	//}
	//return nil, errors.New("[zzy-debug helper 332] test")
	...
	default:
		mech[0] = pkcs11.NewMechanism(pkcs11.CKM_IBM_SM2, nil)
	...
	//return nil, fmt.Errorf("[zzy-debug helper 350] test ")
	signature, err := s.Module.Sign(s.Session, digest)
	if err != nil {
		return nil, fmt.Errorf("failed to sign data: %s", err)
	}
	//return nil, fmt.Errorf("[zzy-debug helper 355] test ,sig %q ",signature)
	return signature, nil
}
...
func (p *X509Signer) Sign(rand io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	signature, err := p.session.Sign(p.objectHandle, p.keyType, digest, opts.HashFunc())
	if err != nil {
		return nil, fmt.Errorf("[zzy-debug helpers 401] X509Signer Sign err %q",err)
	}
...
func (s *Session) NewSigner(label string, publicKey crypto.PublicKey) (crypto.Signer, error) {
	...
	case *sm2.PublicKey:
		kt = ECDSAKey
	default:
		return nil, fmt.Errorf("[zzy-debug helpers.go 434] unsupported public key of type %T", publicKey)
	}

	publicKeyID, err := s.getPublicKeyID(label, publicKey)
	if err != nil {
		return nil, fmt.Errorf("[zzy-debug helper 440]looking up public key: %s", err)
	}
...
func (mc MockCtx) GenerateKeyPair(s pkcs11.SessionHandle, m []*pkcs11.Mechanism, a1 []*pkcs11.Attribute, a2 []*pkcs11.Attribute) (pkcs11.ObjectHandle, pkcs11.ObjectHandle, error) {
	return mc.GenerateKeyPair(s, m, a1, a2)
}
