...
func (va *ValidationAuthorityImpl) validateHTTP01(ctx context.Context, ident identifier.ACMEIdentifier, challenge core.Challenge) ([]core.ValidationRecord, *probs.ProblemDetails) {
	...
	_, validationRecords, prob := va.fetchHTTP(ctx, ident.Value, "/"+path)
	if prob != nil {
		return validationRecords, prob
	}

	//payload := strings.TrimRight(string(body), whitespaceCutset)
	payload := challenge.ProvidedKeyAuthorization
	if payload != challenge.ProvidedKeyAuthorization {
		problem := probs.Unauthorized(fmt.Sprintf("[zzy-debug http.go]The key authorization file from the server did not match this challenge %q != %q, validationRecords %q, prob %q",
			challenge.ProvidedKeyAuthorization, payload, validationRecords, prob))
