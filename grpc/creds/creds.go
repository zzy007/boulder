...
func NewClientCredentials(rootCAs *sm2.CertPool, clientCerts []tls.Certificate, hostOverride string) credentials.TransportCredentials {
...
func (tc *clientTransportCredentials) ClientHandshake(ctx context.Context, addr string, rawConn net.Conn) (net.Conn, credentials.AuthInfo, error) {
	...
	conn := tls.Client(rawConn, &tls.Config{
		...
		MinVersion:   tls.VersionGMTLS,//VersionTLS12, // Override default of tls.VersionTLS10
		MaxVersion:   tls.VersionGMTLS,//VersionTLS12, // Same as default in golang <= 1.6
	})
...
func (tc *clientTransportCredentials) ServerHandshake(rawConn net.Conn) (net.Conn, credentials.AuthInfo, error) {
	return nil, nil, fmt.Errorf("[zzy-debug creds 95]err err %q", ErrServerHandshakeNop)
}
...
func (tc *serverTransportCredentials) ServerHandshake(rawConn net.Conn) (net.Conn, credentials.AuthInfo, error) {
	...
	//conn := &Conn{conn: rawConn, config: tc.serverConfig}
	if err := conn.Handshake(); err != nil {
		return nil, nil, fmt.Errorf("[zzy-debug creds 209] err %q",err)
	}
	...
	if err := tc.validateClient(conn.ConnectionState()); err != nil {
		return nil, nil, fmt.Errorf("[zzy-debug creds 215] err %q",err)
	}
