...
func sigAlgorithmForKey(key *jose.JSONWebKey) (string, error) {
	...
	return "", errors.New("JWK contains unsupported key type (expected SM2-P-256, or RSA, or ECDSA P-256, P-384, or P-521")
...
func checkAlgorithm(key *jose.JSONWebKey, parsedJWS *jose.JSONWebSignature) error {
	...
	//	return fmt.Errorf(
	//		"JWS signature header contains unsupported algorithm %q, expected one of SM2256, RS256, ES256, ES384 or ES512",
	...
	if sigHeaderAlg != expectedAlg {
		return fmt.Errorf("JWS signature header algorithm %q does not match expected algorithm %q for JWK", sigHeaderAlg, expectedAlg)
	}
	if key.Algorithm != "" && key.Algorithm != expectedAlg {
		return fmt.Errorf("JWK key header algorithm %q does not match expected algorithm %q for JWK", key.Algorithm, expectedAlg)
	}
	return nil
}
...
func (wfe *WebFrontEndImpl) parseJWS(body []byte) (*jose.JSONWebSignature, *probs.ProblemDetails) {
	...
	if err := json.Unmarshal(body, &unprotected); err != nil {
		...
		fmt.Println("-----[zzy debug] server verify: parseJWS() error -----")
		return nil, probs.Malformed("Parse error reading JWS [zzy debug verify:316]")
	}
	...
	if err != nil {
		...
		return nil, probs.Malformed(fmt.Sprintf("Parse error reading JWS [zzy-debug verify:346],bodyStr is: %q,error is: %q ",bodyStr ,err.Error()))
	}
	...
	if len(parsedJWS.Signatures) == 0 {
		...
		return nil, probs.Malformed("[zzy-debug verify]POST JWS not signed that signatures ==0")
	}
	if len(parsedJWS.Signatures) == 1 && len(parsedJWS.Signatures[0].Signature) == 0 {
		...
		return nil, probs.Malformed("[zzy-debug verify]POST JWS not signed that signatures ==1 and signatures[0].Signature ==0")
	}
...
func (wfe *WebFrontEndImpl) validJWSForKey(
	...
	case *ecdsa.PublicKey:
		switch k.Curve {
		case sm2.P256Sm2():
			//  对SM2的签名结果验证，先调用本文件的VerifySm2()函数。
			//err1 := wfe.VerifySm2(jwk, jws)
			payload1, err1 := jws.Verify(jwk)
			if err1 != nil {
				wfe.stats.joseErrorCount.With(prometheus.Labels{"type": "JWSSM2VerifyFailed"}).Inc()
				return nil, probs.Malformed(fmt.Sprintf(
					"JWS SM2 verification error : %q", err1))
			} else {

				// Store the verified payload in the logEvent
				logEvent.Payload = string(payload1)
				var parsedBody struct{}
				if err := json.Unmarshal(payload1, &parsedBody); string(payload1) != "" && err != nil {
		...
		default:
			...
			// Store the verified payload in the logEvent
			logEvent.Payload = string(payload2)
			var parsedBody struct{}
			if err := json.Unmarshal(payload2, &parsedBody); string(payload2) != "" && err != nil {
				wfe.stats.joseErrorCount.With(prometheus.Labels{"type": "JWSBodyUnmarshalFailed"}).Inc()
				return nil, probs.Malformed("Request payload did not parse as JSON")
			}
	...
	default:
		...
		// Store the verified payload in the logEvent
		logEvent.Payload = string(payload3)
		var parsedBody struct{}
		if err := json.Unmarshal(payload3, &parsedBody); string(payload3) != "" && err != nil {
			wfe.stats.joseErrorCount.With(prometheus.Labels{"type": "JWSBodyUnmarshalFailed"}).Inc()
			return nil, probs.Malformed("Request payload did not parse as JSON")
		}
...
func (wfe *WebFrontEndImpl) VerifySm2(jwk *jose.JSONWebKey, jws *jose.JSONWebSignature) error{
	signature := jws.Signatures[0]
	signed := signature.Signature
	//ecdsaSig := new(ecdsaSignature)

	r,s,err := sm2.SignDataToSignDigit(signed)
	if err != nil {
		return probs.Malformed(fmt.Sprintf("[zzy-debug verify:622] SM2 SignDataToSignDigit Error： %q", err))
	}
	//if rest, err := asn1.Unmarshal(signed, ecdsaSig); err != nil {
	//	return err
	//} else if len(rest) != 0 {
	//	return errors.New("SM2: trailing data after ECDSA signature")
	//}
	//if ecdsaSig.R.Sign() <= 0 || ecdsaSig.S.Sign() <= 0 {
	//	return errors.New("SM2: ECDSA signature contained zero or negative values")
	//}
	if r.Sign() <= 0 || s.Sign() <= 0 {
		return errors.New("SM2: ECDSA signature contained zero or negative values")
	}

	switch sm2PKey := jwk.Key.(type) {
	case *ecdsa.PublicKey:
		...
		if !sm2.Sm2Verify(sm2pub, jws.UnsafePayloadWithoutVerification(), nil, r, s) {
			return errors.New("SM2: SM2 verification failure")
		}
...
func (wfe *WebFrontEndImpl) validKeyRollover(
	...
	var innerPayload []byte = innerJWS.UnsafePayloadWithoutVerification()
	var err error
	...
	case *ecdsa.PublicKey:
		...
		case sm2.P256Sm2():
			...
			//  对SM2的签名结果验证，先调用本文件的verifySm2()函数。
			err = wfe.VerifySm2(jwk, innerJWS)
			if err != nil {
				wfe.stats.joseErrorCount.With(prometheus.Labels{"type": "Sm2KeyRolloverJWSVerifyFailed"}).Inc()
				return nil, probs.Malformed("SM2 Inner JWS does not verify with embedded JWK")
			}
		default:
