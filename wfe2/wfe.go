...
type WebFrontEndImpl struct {
	...
	issuerCertificates []*sm2.Certificate
...
func (wfe *WebFrontEndImpl) NewAccount(
	...
	response http.ResponseWriter,
	request *http.Request) {
	fmt.Sprintf("-----[zzy debug] server wfe : handle new-account post -----")
...
// the decision.
type authorizedToRevokeCert func(*sm2.Certificate) *probs.ProblemDetails
...
func (wfe *WebFrontEndImpl) processRevocation(
	...
	providedCert, err := sm2.ParseCertificate(revokeRequest.CertificateDER)
	if err != nil {
	...
	parsedCertificate, err := sm2.ParseCertificate(certDER)
	if err != nil {
...
func (wfe *WebFrontEndImpl) revokeCertByKeyID(
	...
	authorizedToRevoke := func(parsedCertificate *sm2.Certificate) *probs.ProblemDetails {
...
func (wfe *WebFrontEndImpl) Certificate(ctx context.Context, logEvent *web.RequestEvent, response http.ResponseWriter, request *http.Request) {
	...
	if len(wfe.certificateChains) > 0 {
		parsedCert, err := sm2.ParseCertificate(cert.DER)
		if err != nil {
...
func (wfe *WebFrontEndImpl) FinalizeOrder(ctx context.Context, logEvent *web.RequestEvent, response http.ResponseWriter, request *http.Request) {
	...
	csr, err := sm2.ParseCertificateRequest(rawCSR.CSR)
	if err != nil {
