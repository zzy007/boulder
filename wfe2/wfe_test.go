...
func TestNewAccount(t *testing.T) {
	...
	fmt.Println("-----[zzy debug] server wfe_test: TestNewAccount() error -----")
	acctErrTests := []newAcctErrorTest{
		...
		// POST, but body that isn't valid JWS
		{
			...
			`{"type":"` + probs.V2ErrorNS + `malformed","detail":"Parse error reading JWS [zzy-debug wfe2\wfe_test:1403]","status":400}`,
		},
...
func TestAccount(t *testing.T) {
	...
	test.AssertUnmarshaledEquals(t,
		responseWriter.Body.String(),
		`{"type":"`+probs.V2ErrorNS+`malformed","detail":"Parse error reading JWS [zzy-debug wfe2\wfe_test:1669]","status":400}`)
	responseWriter.Body.Reset()
...
func TestNewOrder(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			ExpectedBody: `{"type":"` + probs.V2ErrorNS + `malformed","detail":"Parse error reading JWS[zzy-debug wfe2\wfe_test:2228]","status":400}`,
		},
		{
...
func TestFinalizeOrder(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			ExpectedBody: `{"type":"` + probs.V2ErrorNS + `malformed","detail":"Parse error reading JWS[zzy-debug wfe2\wfe_test:2337]","status":400}`,
		},
		{
			Name:         "POST, properly signed JWS, payload isn't valid",
			Request:      signAndPost(t, targetPath, signedURL, "foo", 1, wfe.nonceService),
...
func TestKeyRollover(t *testing.T) {
	...
	test.AssertUnmarshaledEquals(t,
		...
		`{
		  ...
		  "detail": "Parse error reading JWS [zzy-debug wfe2\wfe_test:2456]",
		  "status": 400
		}`)
