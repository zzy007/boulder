...
func (kgc keyGenConfig) validate() error {
	...
	if kgc.Type == "ecdsa" && !allowedCurves[kgc.ECDSACurve] {
		return fmt.Errorf("key.ecdsa-curve can only be 'P-224', 'P-256', 'P-384', or 'P-521' ,Curve %q",kgc.ECDSACurve)
	}
...
func loadCert(filename string) (cert *sm2.Certificate, err error) {
...
func equalPubKeys(a, b interface{}) bool {
	aBytes, err := sm2.MarshalPKIXPublicKey(a)
	if err != nil {
		return false
	}
	bBytes, err := sm2.MarshalPKIXPublicKey(b)
	if err != nil {
		return false
	}
...
func signAndWriteCert(tbs, issuer *sm2.Certificate, subjectPubKey crypto.PublicKey, signer crypto.Signer, certPath string, skipLints []string) error {
	...
	// changes.
	certBytes, err := sm2.CreateCertificate(&failReader{}, tbs, issuer, subjectPubKey, signer)
	if err != nil {
		return fmt.Errorf("failed to create certificate: %s", err)
	}
	log.Printf("[zzy-debug main] created certificate certBytes")
	pemBytes := pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: certBytes})
	log.Printf("Signed certificate PEM:\n%s", pemBytes)
	cert, err := sm2.ParseCertificate(certBytes)
	if err != nil {
...
func intermediateCeremony(configBytes []byte, ct certType) error {
	...
	pub, err := sm2.ParsePKIXPublicKey(pubPEM.Bytes)
	if err != nil {
...
func ocspRespCeremony(configBytes []byte) error {
	...
	var delegatedIssuer *sm2.Certificate
