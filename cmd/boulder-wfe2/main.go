...
func loadCertificateFile(aiaIssuerURL, certFile string) ([]byte, *sm2.Certificate, error) {
	...
	if cert, err = sm2.ParseCertificate(certBlock.Bytes); err != nil {
		return nil, nil, fmt.Errorf(
			"CertificateChain entry for AIA issuer url %q has an "+
...
// certificates.
func loadCertificateChains(chainConfig map[string][]string, requireAtLeastOneChain bool) (map[string][]byte, []*sm2.Certificate, error) {
	results := make(map[string][]byte, len(chainConfig))
	var issuerCerts []*sm2.Certificate
