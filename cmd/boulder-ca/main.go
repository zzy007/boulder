...
func loadCFSSLIssuer(issuerConfig IssuerConfig) (crypto.Signer, *sm2.Certificate, error) {
	cert, err := core.LoadCert(issuerConfig.CertFile)
	if err != nil {
		return nil, nil, fmt.Errorf("zzy-debug boulder-ca main 129: err %s", err)
	}

	signer, err := loadCFSSLSigner(issuerConfig, cert)
	if err != nil {
		return nil, nil, fmt.Errorf("zzy-debug boulder-ca main 134: err %s", err)
	}
...
func loadCFSSLSigner(issuerConfig IssuerConfig, cert *sm2.Certificate) (crypto.Signer, error) {
	if issuerConfig.File != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("zzy-debug boulder-ca 152 err %s", err)
		}
		return signer, nil
	}
	...
	if issuerConfig.ConfigFile != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("zzy-debug boulder-ca 161 err %s", err)
		}
		pkcs11Config = new(pkcs11key.Config)
		err = json.Unmarshal(contents, pkcs11Config)
		if err != nil {
			return nil, fmt.Errorf("zzy-debug boulder-ca 166 err %s", err)
		}
	} else {
