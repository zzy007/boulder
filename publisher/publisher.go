...
func (pub *Impl) SubmitToSingleCTWithResult(ctx context.Context, req *pubpb.Request) (*pubpb.Result, error) {
	cert, err :=  sm2.ParseCertificate(req.Der)
	if err != nil {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("[zzy-debug publisher 242] error: %q", err)
	}
	...
	if err != nil {
		if canceled.Is(err) {
			return nil, fmt.Errorf("[zzy-debug publisher 255] error: %q", err)
		}
		var body string
		...
		pub.log.AuditErrf("Failed to submit certificate to CT log at %s: %s Body=%q",
			ctLog.uri, err, body)
		return nil, fmt.Errorf("[zzy-debug publisher 263] error: %q", err)
	}
...
func (pub *Impl) singleLogSubmit(
	...
	if err != nil {
		...
		return nil, fmt.Errorf("[zzy-debug publisher 303:] error, %q",err)   //"got HTTP status \"400 Bad Request\"" Body=""
	}
	pub.metrics.submissionLatency.With(prometheus.Labels{
		"log":         ctLog.uri,
		"status":      "success",
...
func CreateTestingSignedSCT(req []string, k *ecdsa.PrivateKey, precert bool, timestamp time.Time) []byte {
	...
	// Sign the SCT
	rawKey, _ := sm2.MarshalPKIXPublicKey(&k.PublicKey)
