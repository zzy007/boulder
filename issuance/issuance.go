...
func LoadIssuer(location IssuerLoc) (*sm2.Certificate, crypto.Signer, error) {
...
func loadSigner(location IssuerLoc, cert *sm2.Certificate) (crypto.Signer, error) {
	if location.File != "" {
...
var defaultEKU = []sm2.ExtKeyUsage{
	sm2.ExtKeyUsageServerAuth,
	sm2.ExtKeyUsageClientAuth,
}

func (p *Profile) generateTemplate(clk clock.Clock) *sm2.Certificate {
...
// is well-formed.
func NewIssuer(cert *sm2.Certificate, signer crypto.Signer, profile *Profile, linter *lint.Linter, clk clock.Clock) (*Issuer, error) {
	switch k := cert.PublicKey.(type) {
	case *rsa.PublicKey:
		profile.sigAlg = sm2.SHA256WithRSA
	case *ecdsa.PublicKey:
		switch k.Curve {
		case sm2.P256Sm2():
			profile.sigAlg = sm2.SM2WithSM3
		case elliptic.P256():
			profile.sigAlg = sm2.ECDSAWithSHA256
		case elliptic.P384():
			profile.sigAlg = sm2.ECDSAWithSHA384
		default:
	...
	if profile.useForRSALeaves || profile.useForECDSALeaves {
		if cert.KeyUsage&sm2.KeyUsageCertSign == 0 {
	...
	if cert.KeyUsage&sm2.KeyUsageDigitalSignature == 0 {
...
func (i *Issuer) Algs() []sm2.PublicKeyAlgorithm {
	var algs []sm2.PublicKeyAlgorithm
	if i.Profile.useForRSALeaves {
		algs = append(algs, sm2.RSA)
	}
	if i.Profile.useForECDSALeaves {
		algs = append(algs, sm2.ECDSA)
	}
	return algs
}
...
func generateSKID(pk crypto.PublicKey) ([]byte, error) {
	pkBytes, err := sm2.MarshalPKIXPublicKey(pk)
	if err != nil {
		return nil, err
	}
...
func (i *Issuer) Issue(req *IssuanceRequest) ([]byte, error) {
	...
	switch req.PublicKey.(type) {
	case *rsa.PublicKey:
		template.KeyUsage = sm2.KeyUsageDigitalSignature | sm2.KeyUsageKeyEncipherment
	...
	return sm2.CreateCertificate(rand.Reader, template, i.Cert, req.PublicKey, i.Signer)
}
...
func RequestFromPrecert(precert *sm2.Certificate, scts []ct.SignedCertificateTimestamp) (*IssuanceRequest, error) {
