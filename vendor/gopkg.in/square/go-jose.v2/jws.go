...
func (obj JSONWebSignature) computeAuthData(payload []byte, signature *Signature) ([]byte, error) {
	...
	//authData.WriteString(base64.RawURLEncoding.EncodeToString(obj.protected))
	...
	if needsBase64 {
		authData.WriteString(base64.RawURLEncoding.EncodeToString(obj.payload))
	} else {
		authData.Write(obj.payload)
	}
