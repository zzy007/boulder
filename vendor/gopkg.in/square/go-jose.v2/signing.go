...
func (obj JSONWebSignature) DetachedVerify(payload []byte, verificationKey interface{}) error {
	...
	for _, name := range critical {
		if !supportedCritical[name] {
			return fmt.Errorf("[zzy-debug signing:369]square/go-jose: unsupportedCritical : %v", name)
		}
	}

	input, err := obj.computeAuthData(payload, &signature)
	if err != nil {
		return fmt.Errorf("[zzy-debug signing:375]square/go-jose: computeAuthData error : %v", err)
	}
	...
	return fmt.Errorf("[zzy-debug signing:384]square/go-jose: verifyPayload error: %v" , err)
}
