...
func (kr *KeyRequest) SigAlgo() sm2.SignatureAlgorithm {
	switch kr.Algo() {
	//case "rsa":
	//	switch {
	//	case kr.Size() >= 4096:
...
func ParseRequest(req *CertificateRequest) (csr, key []byte, err error) {
	...
	case *rsa.PrivateKey:
		key = sm2.MarshalPKCS1PrivateKey(priv)
		block := pem.Block{
	...
	case *ecdsa.PrivateKey:
		key, err = marshalECPrivateKeyWithOID(priv)
		if err != nil {
...
func ExtractCertificateRequest(cert *sm2.Certificate) *CertificateRequest {
	req := New()
...
func getHosts(cert *sm2.Certificate) []string {
	var hosts []string
...
func Regenerate(priv crypto.Signer, csr []byte) ([]byte, error) {
	...
	return sm2.CreateCertificateRequest(rand.Reader, req, priv)
}
