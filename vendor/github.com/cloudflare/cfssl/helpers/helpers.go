...
func ParseCertificatesPEM(certsPEM []byte) ([]*sm2.Certificate, error) {
...
func ParseCertificatesDER(certsDER []byte, password string) (certs []*sm2.Certificate, key crypto.Signer, err error) {
	certsDER = bytes.TrimSpace(certsDER)
	pkcs7data, err := pkcs7.ParsePKCS7(certsDER)
	if err != nil {
		var pkcs12data interface{}
		certs = make([]*sm2.Certificate, 1)
		pkcs12data, certs, err = pkcs12.DecodeAll(certsDER, password)

		if err != nil {
			certs, err = sm2.ParseCertificates(certsDER)
			if err != nil {
...
func ParseSelfSignedCertificatePEM(certPEM []byte) (*sm2.Certificate, error) {
...
func PEMToCertPool(pemCerts []byte) (*sm2.CertPool, error) {
	...
	if !certPool.AppendCertsFromPEM(pemCerts) {
		return nil, fmt.Errorf("failed to load cert pool")
	}
...
func ParseCSR(in []byte) (csr *sm2.CertificateRequest, rest []byte, err error) {
	...
	if p != nil {
		...
		csr, err = sm2.ParseCertificateRequest(p.Bytes)
	} else {
		csr, err = sm2.ParseCertificateRequest(in)
	}
...
// locally.
func ParseCSRPEM(csrPEM []byte) (*sm2.CertificateRequest, error) {
	block, _ := pem.Decode([]byte(csrPEM))
	if block == nil {
		return nil, cferr.New(cferr.CSRError, cferr.DecodeFailed)
	}
	csrObject, err := sm2.ParseCertificateRequest(block.Bytes)
...
func SignerAlgo(priv crypto.Signer) sm2.SignatureAlgorithm {
	...
	case *rsa.PublicKey:
		...
		switch {
		case bitLength >= 4096:
			return sm2.SHA512WithRSA
		case bitLength >= 3072:
			return sm2.SHA384WithRSA
		case bitLength >= 2048:
			return sm2.SHA256WithRSA
		default:
			return sm2.SHA1WithRSA
		}
	case *ecdsa.PublicKey:
		...
		//case elliptic.P521():
		//	return sm2.ECDSAWithSHA512
		//case elliptic.P384():
		//	return sm2.ECDSAWithSHA384
		//case elliptic.P256():
		//	return sm2.ECDSAWithSHA256
		default:
			return sm2.ECDSAWithSHA1
		}
	default:
