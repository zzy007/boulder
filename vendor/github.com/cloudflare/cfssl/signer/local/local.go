...
func NewSigner(priv crypto.Signer, cert *sm2.Certificate, sigAlgo sm2.SignatureAlgorithm, policy *config.Signing) (*Signer, error) {
	if policy == nil {
		policy = &config.Signing{
...
func (s *Signer) lint(template sm2.Certificate, errLevel lint.LintStatus, lintRegistry lint.Registry) error {
	...
	default:
		...
		return fmt.Errorf("[zzy-debug cfssl sign local switch 165 err:")
	}


	prelintBytes, err := sm2.CreateCertificate(rand.Reader, &template, s.ca, template.PublicKey, s.lintPriv)
	if err != nil {
		return fmt.Errorf("[zzy-debug cfssl sign local CreateCertificate 171 err: %s", err)
	}
	prelintCert, err := zx509.ParseCertificate(prelintBytes)
	if err != nil {
		return fmt.Errorf("[zzy-debug cfssl sign local ParseCertificate 175 error %s", err)
	}
	errorResults := map[string]lint.LintResult{}
...
func (s *Signer) sign(template *sm2.Certificate, lintErrLevel lint.LintStatus, lintRegistry lint.Registry) (cert []byte, err error) {
	var initRoot bool
	if s.ca == nil {
		if !template.IsCA {
			err = cferr.New(cferr.PolicyError, cferr.InvalidRequest)
			return nil,errors.New("[zzy-debug cfssl sign local 195] error")
		}
...
func (s *Signer) Sign(req signer.SignRequest) (cert []byte, err error) {
	...
	if err != nil {
		return nil, errors.New("[zzy-debug cfssl sign local288] error")
	}
	...
	if err != nil {
		return nil, errors.New("[zzy-debug cfssl sign local 303] error")
	}
	...
	if err != nil {
		return nil,  errors.New("[zzy-debug cfssl sign local 439] error")
	}
	...
	if err != nil {
		return nil, errors.New("[zzy-debug cfssl sign local 499] error")
	}
	...
	if s.dbAccessor != nil {
		...
		if err != nil {
			return nil, errors.New("[zzy-debug cfssl sign local 521] error")
		}
...
func (s *Signer) SignFromPrecert(precert *sm2.Certificate, scts []ct.SignedCertificateTimestamp) ([]byte, error) {
