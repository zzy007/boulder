...
func (c *JSONClient) PostAndParse(ctx context.Context, path string, req, rsp interface{}) (*http.Response, []byte, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("[zzy-debug google certificate-transparency-go client 220]error %q",err)
	}
	fullURI := fmt.Sprintf("%s%s", c.uri, path)
	httpReq, err := http.NewRequest(http.MethodPost, fullURI, bytes.NewReader(postBody))
	if err != nil {
		return nil, nil, fmt.Errorf("[zzy-debug google certificate-transparency-go client 225]error %q",err)
	}
	if len(c.userAgent) != 0 {
		httpReq.Header.Set("User-Agent", c.userAgent)
	}
	...
	if err != nil {
		if httpRsp != nil {
			return nil, nil, RspError{StatusCode: httpRsp.StatusCode, Body: body, Err: fmt.Errorf("[zzy-debug google certificate-transparency-go client 242]error %q",err)}
		}
		return nil, nil, fmt.Errorf("[zzy-debug google certificate-transparency-go client 244]error %q",err)
	}

	if httpRsp.StatusCode == http.StatusOK {
		if err = json.Unmarshal(body, &rsp); err != nil {
			return nil, nil, RspError{StatusCode: httpRsp.StatusCode, Body: body, Err: fmt.Errorf("[zzy-debug google certificate-transparency-go client 249]error %q",err)}
		}
	}
	return httpRsp, body, nil
}
	...
	for {
		...
		if err != nil {
			...
			if err == context.Canceled || err == context.DeadlineExceeded {
				return nil, nil, fmt.Errorf("[zzy-debug google certificate-transparency-go client 283]error %q",err)
			}
			...
			default:
				return nil, nil, RspError{
					...
					Body:       body,
					Err: fmt.Errorf("[zzy-debug client]got HTTP status %q，uRI %s ，path %s，postBody %q", httpRsp.Status, c.uri, path),
				}
			}
		}
