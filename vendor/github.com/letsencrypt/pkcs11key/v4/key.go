...
func (ps *Key) Sign(rand io.Reader, msg []byte, opts crypto.SignerOpts) (signature []byte, err error) {
	...
	default:
		isECDSA = true
		mechanism = []*pkcs11.Mechanism{pkcs11.NewMechanism(pkcs11.CKM_IBM_SM2, nil)}
		signatureInput = msg
	}
	...
	if err != nil {
		return nil, fmt.Errorf("[zzy-debug pkcs11key 481]pkcs11key: sign init: %s", err)
	}

	signature, err = ps.module.Sign(*ps.session, signatureInput)
	if err != nil {
		return nil, fmt.Errorf("[zzy-debug pkcs11key 481]pkcs11key: sign: %s", err)
	}
